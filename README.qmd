...
This allows control over the widths with a single argument.
In the plot below, for example, we double the thickness of the lines in the baseline by setting the `width_multiplier` to 2.5, making the lines more visible.
...
scale_line_widths = function(
  ...
  if(is.null(width_multiplier)) {
    width_multiplier = max_width / max_width_low
  }
  x_no_outliers = x |>
    sf::st_drop_geometry() |>
  ...
  maximums = x_normalized |>
    sapply(max)
  # summary(x_normalized)
  minimum_value_allowed = min(maximums) / width_multiplier
